            ...
            with open(self.gripperEncoderSyncDirs[i], 'r') as lines:
                ...
                for t, line in enumerate(lines):
                    ...
                    with open(os.path.join(self.gripperEncoderDirs[i], line), 'r') as file:
                        data = json.load(file)
                        # Pika SDK only provides distance, not angle. Use 0.0 as placeholder
                        data_dict[f'gripper/encoderAngle/{self.args.gripperEncoderNames[i]}'].append(data.get('angle', 0.0))
